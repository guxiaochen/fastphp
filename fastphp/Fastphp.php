...
class Fastphp
{
    ...
    **/
    ...
    // 路由处理,重定向、如果访问目录不存在，会重定向到index.php通过request_url获取域名后面的参数进行解析
    /*
    * 截取Url,并解析出控制器名、方法和Url参数
    ...
    */
    public function route()
    {
        ...
        // 查找字符首次出现的位置,返回数字或者false
        $position = strpos($url, '?');
        $url = $position === false ? $url : substr($url, 0, $position);

        // 也可以这样访问 index.php/{controller}/{action}
    ...
    * 环境检查
    * 开发者模式或者调试模式
    * 1、开发者模式，显示全部错误，display_errors=On
    * 2、线上模式，记录日志，错误提示500，display_errors=Off,log_errors=On
    */
